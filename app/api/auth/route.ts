...
async function callMsg91Api(url: string, method: 'GET' | 'POST' = 'GET', data: any = null) {
  try {
    ...
    return { 
      success: false, 
      error: error.response?.data || 'Failed to communicate with MSG91' 
    };
  }
}
    ...
    switch (action) {
      ...
      case 'resend': {
        ...
        if (result.success) {
          ...
          return NextResponse.json({ 
            success: false,
            message: result.error?.message || "Failed to resend OTP",
      ...
      case 'verify': {
        ...
        if (result.success && result.data.type === 'success') {
          ...
          const errorMessage = result.data?.message || result.error?.message;
